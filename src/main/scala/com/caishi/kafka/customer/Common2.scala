...
val commonEventData = lines.filter(checkDataType(_,"topic_common_event")).foreachRDD(rdd => saveToParquet(rdd,"topic_common_event",pathPre))
  ...
  def saveToParquet(rdd: RDD[String], dataType: String,pathPre : String): Unit = {
    ...
    if(df.count() > 0){
      ...
      try {
        df.printSchema();
        df.show()
        df.select(df("data").toString()).show()

//        df.write.format("parquet").mode(SaveMode.Overwrite).save(dirname)
        df.write.format("parquet").mode(SaveMode.Append).save("file:///root/t.x")
      }catch {
        case e: Throwable =>
